...
import { UserPlus, Users, Send, FilePlus2, Link2, Eye, Loader2 } from "lucide-react";
...
export default function AssessmentBuilder() {
    ...
    const handleInvite = async (email, role) => {
        ...
        try {
            ...
            fetchAssessmentData(); // Re-fetch to show the updated list
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-indigo-100 py-10 px-6">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                {/* LEFT PANEL */}
                <div className="md:col-span-1 bg-white rounded-2xl shadow-xl p-6 border border-indigo-100 space-y-6">
                    <motion.div whileHover={{ scale: 1.02 }} className="bg-gradient-to-r from-indigo-100 to-indigo-200 p-4 rounded-xl">
                        ...
                        {interviewers.length > 0 ? (
                            <ul className="space-y-2">{interviewers.map((i, idx) => <li key={idx} className="flex justify-between items-center p-2 bg-indigo-50 rounded-md text-sm text-indigo-800 shadow-sm">{i.name}<span className={`text-xs font-semibold ${i.status === "Accepted" ? "text-emerald-600" : "text-indigo-400"}`}>{i.status}</span></li>)}</ul>
                ...
                <div className="md:col-span-2 bg-white rounded-2xl shadow-xl p-8 border border-indigo-100 space-y-6">
                    <h1 className="text-3xl font-bold text-indigo-800">{isCreateMode ? "Create New Assessment" : assessment.name}</h1>
                    <div className="space-y-4">
                        <label className="block text-sm font-semibold text-indigo-700">Assessment Name</label>
                        <input type="text" value={assessment.name} onChange={(e) => setAssessment({ ...assessment, name: e.target.value })} placeholder="e.g., Frontend Developer Round 1" disabled={!isCreateMode} className="w-full border border-indigo-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed" />
                        <label className="block text-sm font-semibold text-indigo-700">Description</label>
                        <textarea value={assessment.description} onChange={(e) => setAssessment({ ...assessment, description: e.target.value })} placeholder="Describe the focus, topics, or goals..." rows="3" disabled={!isCreateMode} className="w-full border border-indigo-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed" />
                    </div>

                    <fieldset disabled={isCreateMode} className="disabled:opacity-50">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <h3 className="text-sm font-semibold text-indigo-700 flex items-center gap-2"><Users size={16} /> Invite Interviewers</h3>
                                <div className="flex gap-2"><input type="text" value={inviteInput} onChange={(e) => setInviteInput(e.target.value)} placeholder="Enter email" className="flex-1 border border-indigo-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" /><button onClick={() => handleInvite(inviteInput, 'interviewer')} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition"> <Send size={14} /> Invite </button></div>
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-sm font-semibold text-purple-700 flex items-center gap-2"><UserPlus size={16} /> Invite Candidates</h3>
                                <div className="flex gap-2"><input type="text" value={candidateInput} onChange={(e) => setCandidateInput(e.target.value)} placeholder="Enter candidate email" className="flex-1 border border-purple-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" /><button onClick={() => handleInvite(candidateInput, 'candidate')} className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition"> <Send size={14} /> Invite </button></div>
                            </div>
                        </div>
                        <div className="space-y-2 pt-4">
                            <h3 className="text-sm font-semibold text-indigo-700 flex items-center gap-2"><FilePlus2 size={16} /> Add Questions via URL</h3>
                            <div className="flex gap-2"><input type="text" value={questionUrl} onChange={(e) => setQuestionUrl(e.target.value)} placeholder="Paste question URL" className="flex-1 border border-indigo-200 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" /><button onClick={handleAddQuestion} className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition"> <Link2 size={14} /> Add </button></div>
